...
async function main() {
  ...
  console.log("PlatformToken 合约已部署到:", await platformToken.getAddress());
  ...
  const market = await Market.deploy(
    ...
    await platformToken.getAddress() // platformTokenContract
  );
  ...
  // 设置 PlatformToken 的所有者为 Market 合约
  await platformToken.transferOwnership(await market.getAddress());
  console.log("PlatformToken 合约所有权已转移给 Market 合约");
  ...
  if (hre.network.name !== "hardhat" && hre.network.name !== "localhost") {
    ...
    await hre.run("verify:verify", {
      address: await platformToken.getAddress(),
  ...
  console.log("部署完成！");
  console.log("PlatformToken 合约地址:", await platformToken.getAddress());
